...
class SyncRandomSizeHook(Hook):
    ...
    def __init__(self, ratio_range=(14, 26), img_scale=(640, 640), interval
        =1, device='cuda'):
        print('Filip YuNet Minify: Function fidx=0 __init__ called in mmdet/core/hook/sync_random_size_hook.py:L33 ')
        warnings.warn(
            "DeprecationWarning: SyncRandomSizeHook is deprecated. Please use Resize pipeline to achieve similar functions. Due to the multi-process dataloader, its behavior is different from YOLOX's official implementation, the official is to change the size every fixed iteration interval and what we achieved is a fixed epoch interval."
            )
    ...
    def after_train_epoch(self, runner):
        print('Filip YuNet Minify: Function fidx=1 after_train_epoch called in mmdet/core/hook/sync_random_size_hook.py:L52 ')
        """Change the dataset output image size."""
        if self.ratio_range is not None and (runner.epoch + 1
            ) % self.interval == 0:
            ...
            runner.data_loader.dataset.update_dynamic_scale((tensor[0].item
                (), tensor[1].item()))

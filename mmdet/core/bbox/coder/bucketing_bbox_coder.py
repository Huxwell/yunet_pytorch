...
class BucketingBBoxCoder(BaseBBoxCoder):
    ...
    def __init__(self, num_buckets, scale_factor, offset_topk=2,
        offset_upperbound=1.0, cls_ignore_neighbor=True, clip_border=True):
        print('Filip YuNet Minify: Function fidx=0 __init__ called in mmdet/core/bbox/coder/bucketing_bbox_coder.py:L35 ')
    ...
    def encode(self, bboxes, gt_bboxes):
        print('Filip YuNet Minify: Function fidx=1 encode called in mmdet/core/bbox/coder/bucketing_bbox_coder.py:L50 ')
        ...
        encoded_bboxes = bbox2bucket(bboxes, gt_bboxes, self.num_buckets,
            self.scale_factor, self.offset_topk, self.offset_upperbound,
            self.cls_ignore_neighbor)
        return encoded_bboxes

    def decode(self, bboxes, pred_bboxes, max_shape=None):
        print('Filip YuNet Minify: Function fidx=2 decode called in mmdet/core/bbox/coder/bucketing_bbox_coder.py:L72 ')
        ...
        assert cls_preds.size(0) == bboxes.size(0) and offset_preds.size(0
            ) == bboxes.size(0)
        decoded_bboxes = bucket2bbox(bboxes, cls_preds, offset_preds, self.
            num_buckets, self.scale_factor, max_shape, self.clip_border)
        return decoded_bboxes


@mmcv.jit(coderize=True)
def generat_buckets(proposals, num_buckets, scale_factor=1.0):
    print('Filip YuNet Minify: Function fidx=3 generat_buckets called in mmdet/core/bbox/coder/bucketing_bbox_coder.py:L96 ')
    ...
    l_buckets = px1[:, (None)] + (0.5 + torch.arange(0, side_num).to(
        proposals).float())[(None), :] * bucket_w[:, (None)]
    r_buckets = px2[:, (None)] - (0.5 + torch.arange(0, side_num).to(
        proposals).float())[(None), :] * bucket_w[:, (None)]
    t_buckets = py1[:, (None)] + (0.5 + torch.arange(0, side_num).to(
        proposals).float())[(None), :] * bucket_h[:, (None)]
    d_buckets = py2[:, (None)] - (0.5 + torch.arange(0, side_num).to(
        proposals).float())[(None), :] * bucket_h[:, (None)]
    return bucket_w, bucket_h, l_buckets, r_buckets, t_buckets, d_buckets


@mmcv.jit(coderize=True)
def bbox2bucket(proposals, gt, num_buckets, scale_factor, offset_topk=2,
    offset_upperbound=1.0, cls_ignore_neighbor=True):
    print('Filip YuNet Minify: Function fidx=4 bbox2bucket called in mmdet/core/bbox/coder/bucketing_bbox_coder.py:L145 ')
    ...
    gt = gt.float()
    bucket_w, bucket_h, l_buckets, r_buckets, t_buckets, d_buckets = (
    ...
    l_topk, l_label = l_offsets.abs().topk(offset_topk, dim=1, largest=
        False, sorted=True)
    r_topk, r_label = r_offsets.abs().topk(offset_topk, dim=1, largest=
        False, sorted=True)
    t_topk, t_label = t_offsets.abs().topk(offset_topk, dim=1, largest=
        False, sorted=True)
    d_topk, d_label = d_offsets.abs().topk(offset_topk, dim=1, largest=
        False, sorted=True)
    ...
    offsets_weights = torch.cat([offset_l_weights, offset_r_weights,
        offset_t_weights, offset_d_weights], dim=-1)
    side_num = int(np.ceil(num_buckets / 2.0))
    labels = torch.stack([l_label[:, (0)], r_label[:, (0)], t_label[:, (0)],
        d_label[:, (0)]], dim=-1)
    ...
    bucket_cls_weights = torch.cat([bucket_cls_l_weights,
        bucket_cls_r_weights, bucket_cls_t_weights, bucket_cls_d_weights],
        dim=-1)
    if cls_ignore_neighbor:
        bucket_cls_weights = (~((bucket_cls_weights == 1) & (bucket_labels ==
            0))).float()
    else:
...
@mmcv.jit(coderize=True)
def bucket2bbox(proposals, cls_preds, offset_preds, num_buckets,
    scale_factor=1.0, max_shape=None, clip_border=True):
    print('Filip YuNet Minify: Function fidx=5 bucket2bbox called in mmdet/core/bbox/coder/bucketing_bbox_coder.py:L269 ')
    ...
    bboxes = torch.cat([x1[:, (None)], y1[:, (None)], x2[:, (None)], y2[:,
        (None)]], dim=-1)

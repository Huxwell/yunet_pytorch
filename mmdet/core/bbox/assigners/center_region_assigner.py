...
def scale_boxes(bboxes, scale):
    print('Filip YuNet Minify: Function fidx=0 scale_boxes called in mmdet/core/bbox/assigners/center_region_assigner.py:L10 ')
...
def is_located_in(points, bboxes):
    print('Filip YuNet Minify: Function fidx=1 is_located_in called in mmdet/core/bbox/assigners/center_region_assigner.py:L37 ')
    ...
    return (points[:, (0)].unsqueeze(1) > bboxes[:, (0)].unsqueeze(0)) & (
        points[:, (0)].unsqueeze(1) < bboxes[:, (2)].unsqueeze(0)) & (points
        [:, (1)].unsqueeze(1) > bboxes[:, (1)].unsqueeze(0)) & (points[:, (
        1)].unsqueeze(1) < bboxes[:, (3)].unsqueeze(0))


def bboxes_area(bboxes):
    print('Filip YuNet Minify: Function fidx=2 bboxes_area called in mmdet/core/bbox/assigners/center_region_assigner.py:L55 ')
...
class CenterRegionAssigner(BaseAssigner):
    ...
    def __init__(self, pos_scale, neg_scale, min_pos_iof=0.01,
        ignore_gt_scale=0.5, foreground_dominate=False, iou_calculator=dict
        (type='BboxOverlaps2D')):
        print('Filip YuNet Minify: Function fidx=3 __init__ called in mmdet/core/bbox/assigners/center_region_assigner.py:L94 ')
    ...
    def get_gt_priorities(self, gt_bboxes):
        print('Filip YuNet Minify: Function fidx=4 get_gt_priorities called in mmdet/core/bbox/assigners/center_region_assigner.py:L108 ')
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        print('Filip YuNet Minify: Function fidx=5 assign called in mmdet/core/bbox/assigners/center_region_assigner.py:L126 ')
        """Assign gt to bboxes.
        ...
        bbox_and_gt_core_overlaps = self.iou_calculator(bboxes, gt_core,
            mode='iof')
        is_bbox_in_gt_core = is_bbox_in_gt & (bbox_and_gt_core_overlaps >
            self.min_pos_iof)
        is_bbox_in_gt_shadow = self.iou_calculator(bboxes, gt_shadow, mode=
            'iof') > self.min_pos_iof
        ...
        if num_gts == 0 or num_bboxes == 0:
            ...
            assigned_gt_ids, pixels_in_gt_shadow = (self.
                assign_one_hot_gt_indices(is_bbox_in_gt_core,
                is_bbox_in_gt_shadow, gt_priority=sort_idx))
        if gt_bboxes_ignore is not None and gt_bboxes_ignore.numel() > 0:
            gt_bboxes_ignore = scale_boxes(gt_bboxes_ignore, scale=self.
                ignore_gt_scale)
        ...
        if gt_labels is not None:
            ...
            pos_inds = torch.nonzero(assigned_gt_ids > 0, as_tuple=False
                ).squeeze()
            if pos_inds.numel() > 0:
                assigned_labels[pos_inds] = gt_labels[assigned_gt_ids[
                    pos_inds] - 1]
            shadowed_pixel_labels = pixels_in_gt_shadow.clone()
            if pixels_in_gt_shadow.numel() > 0:
                pixel_idx, gt_idx = pixels_in_gt_shadow[:, (0)
                    ], pixels_in_gt_shadow[:, (1)]
                ...
                override = assigned_labels[pixel_idx] == shadowed_pixel_labels[
                    :, (1)]
        ...
        assign_result = AssignResult(num_gts, assigned_gt_ids, None, labels
            =assigned_labels)
        assign_result.set_extra_property('shadowed_labels',
            shadowed_pixel_labels)
        return assign_result

    def assign_one_hot_gt_indices(self, is_bbox_in_gt_core,
        is_bbox_in_gt_shadow, gt_priority=None):
        print('Filip YuNet Minify: Function fidx=6 assign_one_hot_gt_indices called in mmdet/core/bbox/assigners/center_region_assigner.py:L257 ')
        ...
        if gt_priority is None:
            gt_priority = torch.arange(num_gts, device=is_bbox_in_gt_core.
                device)
        ...
        matched_bbox_gt_inds = torch.nonzero(is_bbox_in_gt_core, as_tuple=False
            )[:, (1)]
        ...
        shadowed_gt_inds = torch.cat((shadowed_gt_inds, torch.nonzero(
            is_bbox_in_gt_core, as_tuple=False)), dim=0)

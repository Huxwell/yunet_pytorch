...
def generate_inputs_and_wrap_model(config_path, checkpoint_path,
    input_config, cfg_options=None):
    print('Filip YuNet Minify: Function fidx=0 generate_inputs_and_wrap_model called in mmdet/core/export/pytorch2onnx.py:L10 ')
    ...
    """
    model = build_model_from_cfg(config_path, checkpoint_path, cfg_options=
        cfg_options)
...
def build_model_from_cfg(config_path, checkpoint_path, cfg_options=None):
    print('Filip YuNet Minify: Function fidx=1 build_model_from_cfg called in mmdet/core/export/pytorch2onnx.py:L65 ')
...
def preprocess_example_input(input_config):
    print('Filip YuNet Minify: Function fidx=2 preprocess_example_input called in mmdet/core/export/pytorch2onnx.py:L102 ')
    ...
    one_img = torch.from_numpy(one_img).unsqueeze(0).float().requires_grad_(
        True)
    _, C, H, W = input_shape
    one_meta = {'img_shape': (H, W, C), 'ori_shape': (H, W, C), 'pad_shape':
        (H, W, C), 'filename': '<demo>.png', 'scale_factor': np.ones(4,
        dtype=np.float32), 'flip': False, 'show_img': show_img,
        'flip_direction': None}

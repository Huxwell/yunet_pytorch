...
async def completed(trace_name='', name='', sleep_interval=0.05, streams:
    ...
    end_events = [torch.cuda.Event(enable_timing=DEBUG_COMPLETED_TIME) for
        _ in streams]
...
@contextlib.asynccontextmanager
async def concurrent(streamqueue: asyncio.Queue, trace_name='concurrent',
    name='stream'):

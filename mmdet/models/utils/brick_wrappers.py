...
def adaptive_avg_pool2d(input, output_size):
    print('Filip YuNet Minify: Function fidx=0 adaptive_avg_pool2d called in mmdet/models/utils/brick_wrappers.py:L15 ')
...
class AdaptiveAvgPool2d(nn.AdaptiveAvgPool2d):
    ...
    def forward(self, x):
        print('Filip YuNet Minify: Function fidx=1 forward called in mmdet/models/utils/brick_wrappers.py:L35 ')
        if x.numel() == 0 and obsolete_torch_version(TORCH_VERSION, (1, 9)):
            output_size = self.output_size
            if isinstance(output_size, int):
                output_size = [output_size, output_size]
            else:
                output_size = [(v if v is not None else d) for v, d in zip(

...
def pvt_convert(ckpt):
    print('Filip YuNet Minify: Function fidx=0 pvt_convert called in mmdet/models/utils/ckpt_convert.py:L12 ')
...
def swin_converter(ckpt):
    print('Filip YuNet Minify: Function fidx=1 swin_converter called in mmdet/models/utils/ckpt_convert.py:L85 ')
    new_ckpt = OrderedDict()

    def correct_unfold_reduction_order(x):
        out_channel, in_channel = x.shape
        x = x.reshape(out_channel, 4, in_channel // 4)
        x = x[:, ([0, 2, 1, 3]), :].transpose(1, 2).reshape(out_channel,
            in_channel)
        return x

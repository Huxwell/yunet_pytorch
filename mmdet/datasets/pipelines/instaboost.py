...
class InstaBoost:
    ...
    def __init__(self, action_candidate=('normal', 'horizontal', 'skip'),
        action_prob=(1, 0, 0), scale=(0.8, 1.2), dx=15, dy=15, theta=(-1, 1
        ), color_prob=0.5, hflag=False, aug_ratio=0.5):
        print('Filip YuNet Minify: Function fidx=0 __init__ called in mmdet/datasets/pipelines/instaboost.py:L35 ')
        try:
        ...
        self.cfg = instaboost.InstaBoostConfig(action_candidate,
            action_prob, scale, dx, dy, theta, color_prob, hflag)
        self.aug_ratio = aug_ratio

    def _load_anns(self, results):
        print('Filip YuNet Minify: Function fidx=1 _load_anns called in mmdet/datasets/pipelines/instaboost.py:L56 ')
        ...
        for i in range(n):
            ...
            anns.append({'category_id': label, 'segmentation': mask, 'bbox':
                bbox})
        return anns

    def _parse_anns(self, results, anns, img):
        print('Filip YuNet Minify: Function fidx=2 _parse_anns called in mmdet/datasets/pipelines/instaboost.py:L78 ')
        gt_bboxes = []
        gt_labels = []
        gt_masks_ann = []
        for ann in anns:
    ...
    def __call__(self, results):
        print('Filip YuNet Minify: Function fidx=3 __call__ called in mmdet/datasets/pipelines/instaboost.py:L99 ')
        img = results['img']
        ori_type = img.dtype
        ...
        if np.random.choice([0, 1], p=[1 - self.aug_ratio, self.aug_ratio]):
            ...
            anns, img = instaboost.get_new_data(anns, img.astype(np.uint8),
                self.cfg, background=None)
        results = self._parse_anns(results, anns, img.astype(ori_type))
        return results

    def __repr__(self):
        print('Filip YuNet Minify: Function fidx=4 __repr__ called in mmdet/datasets/pipelines/instaboost.py:L115 ')
